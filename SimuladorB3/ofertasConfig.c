    ...
    if( recupera_ofertas( &atualVenda, &atualCompra ) ){
        ...
        while( ofertaVendaAtual != NULL || ofertaCompraAtual != NULL ){
            if( ofertaVendaAtual != NULL ){
                printf( "%10d%12.2f\t\t", ofertaVendaAtual->quantidade, ofertaVendaAtual->valor );
                        ofertaVendaAtual = ofertaVendaAtual->next;
            }else{printf( "%10s%13s\t\t", " ", " "); }
            while( ofertaCompraAtual != NULL ){
                printf( "%10d%11.2f", ofertaCompraAtual->quantidade, ofertaCompraAtual->valor );
...
bool recupera_ofertas( acao **inicioAcaoVenda, acao **inicioAcaoCompra ){
    ...
    FILE *arquivoAcoesValorVenda = fopen( "arquivos/acoesValorVenda.txt", "r" ),
         *arquivoAcoesValorCompra = fopen( "arquivos/acoesValorCompra.txt", "r" ),
         *arquivoDadosConfig = fopen( "arquivos/dadosConfig.txt", "r" );
...
bool retira_oferta( ){
    ...
    if( !atual || strcmp( atual->codigo, atualVenda->identificacao.codigo ) != 0 ){
        dados.quantidade_de_acoes--;
...
bool salva_ofertas( acao **inicioVenda, acao **inicioCompra ){
    ...
    FILE *arquivoAcoesValorVenda = fopen( "arquivos/acoesValorVenda.txt", "w" ),
         *arquivoAcoesValorCompra = fopen( "arquivos/acoesValorCompra.txt", "w" ),
         *arquivoDadosConfig = fopen( "arquivos/dadosConfig.txt", "w" );
...
bool gerador_de_cotacao( unsigned posicao ){
    acao *inicioAcaoCompra = NULL,
