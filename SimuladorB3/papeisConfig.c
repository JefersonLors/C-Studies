...
bool adiciona_papel( ){
    ...
    FILE *quantidadePapel = fopen( "arquivos/papeis.txt", "r" );
...
bool salva_papeis( papel *head ){
    ...
    FILE *arquivoPapeis = fopen( "arquivos/papeis.txt", "w" ),
         *arquivoConfig = fopen( "arquivos/dadosConfig.txt", "w" );
...
bool recupera_papeis( papel **head ){
    papel *atual,
          *proximo;
    FILE *arquivoPapeis = fopen( "arquivos/papeis.txt", "r" ),
         *arquivoConfig = fopen( "arquivos/dadosConfig.txt", "r" );
...
bool listar_papeis( ){
    ...
    if( recupera_papeis( &head ) ){
        ...
        printf( "#%17s        %10s        %11s\t\t\n",
                "COTAÇÃO", "CÓDIGO", "NOME\n" );
        do{ printf( "%-*dR$%-*.2f%-*s%-*s\n",
            11, contador++, TAM_COT+9, primeiro->cotacao, 20, primeiro->codigo, TAM_NOME_PREGAO,
